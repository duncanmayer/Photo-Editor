...
IMPLEMENTATION DETAILS FOR EXTRA CREDIT:
    What we added:  A new command, DownScale, that downscales an image to a given width and height.
    We also added to our relevant command maps the DownScale function object, in order for it to be
    read by the Scanner in the command line implementation of our program.  We added the ability for
    Users to downscale images in our GUI as well, by creating two new input boxes similar in design
    to our Brighten command, allowing users to input their new desired width and height.  We chose
    to add to these code aspects rather than rewriting classes or extending interfaces because the
    intentional design of this code was to make it as easy as possible to add new function objects
    by using these command maps, rather than hard coding switch statements.  The ease with which we
    created this new command is a testament to that.
